...
$id = $_GET['id'];
$query_tampil = "SELECT * FROM tb_seni_budaya WHERE id = '$id'";
...
if (isset($_POST['update'])) {
    ...
    // Pertahankan path lama sebagai default
    $foto_path = $data['foto_path'];
    $audio_path = $data['audio_path'];

    // 1. Proses Update File Foto
    ...
    $query_update = "UPDATE tb_seni_budaya SET kategori='$kategori', judul='$judul', isi='$isi', foto_path='$foto_path', audio_path='$audio_path' WHERE id='$id'";
    
    if (mysqli_query($koneksi, $query_update)) {
        echo "<script>alert('Data berhasil diperbarui!'); window.location='seni_kelola.php';</script>";
    } else {
...
<head>
    ...
    <title>Edit Seni & Budaya - Admin</title>
...
<div class="sidebar">
    ...
    <div class="sidebar-menu">
        ...
        <a href="seni_kelola.php" class="active">🎭 Kelola Seni & Budaya</a>
...
<div class="main-content">
    <div class="form-card">
        <div class="form-header">
            <h3>Edit Seni & Budaya</h3>
            <p>Perbarui informasi karya seni, alat musik, atau tarian Adonara.</p>
        </div>

        <form action="" method="POST" enctype="multipart/form-data">
            
            <div class="form-group">
                <label for="kategori">Kategori Kesenian</label>
                <select name="kategori" id="kategori" class="form-control" required>
                    <option value="tari" <?= ($data['kategori'] == 'tari') ? 'selected' : ''; ?>>Seni Tari</option>
                    <option value="musik" <?= ($data['kategori'] == 'musik') ? 'selected' : ''; ?>>Seni Musik</option>
                    <option value="tenun" <?= ($data['kategori'] == 'tenun') ? 'selected' : ''; ?>>Budaya Tenun</option>
                    <option value="kriya" <?= ($data['kategori'] == 'kriya') ? 'selected' : ''; ?>>Seni Kriya / Ukir</option>
                </select>
            </div>

            <div class="form-group">
                <label for="judul">Judul Karya / Kesenian</label>
                <input type="text" name="judul" id="judul" class="form-control" value="<?= $data['judul']; ?>" required>
            </div>

            <div class="form-group">
                <label for="isi">Isi Plakat (Penjelasan Kesenian)</label>
                <textarea name="isi" id="isi" class="form-control" rows="10" required><?= $data['isi']; ?></textarea>
            </div>
            
            <div class="form-group">
                <label for="foto_file">Update Foto Karya Seni (JPG/PNG)</label>
                ...
                <?php if(!empty($data['foto_path'])): ?>
                    <div class="info-box info-box-success">
                        <strong>Foto Saat Ini:</strong> Sudah ada foto yang tersimpan. Kosongkan jika tidak ingin mengubahnya.
                    </div>
                <?php else: ?>
                    <div class="info-box">
                        <strong>Foto Belum Tersedia.</strong> Unggah foto untuk bingkai museum pameran ini.
                    </div>
                <?php endif; ?>
            </div>

            <div class="form-group">
                <label for="audio_file">Update Audio Museum Guide (.mp3)</label>
                <input type="file" name="audio_file" id="audio_file" class="form-control" accept=".mp3">
                <?php if(!empty($data['audio_path'])): ?>
                    <div class="info-box info-box-success">
                        <strong>Audio Saat Ini:</strong> Sudah ada file audio yang tersimpan. Kosongkan jika tidak ingin mengubahnya.
                    </div>
                <?php else: ?>
                    <div class="info-box">
            ...
            <div class="btn-container">
                <a href="seni_kelola.php" class="btn btn-cancel">Batal</a>
