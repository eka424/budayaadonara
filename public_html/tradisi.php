...
// Mengambil data kontak untuk footer
$query_kontak = "SELECT * FROM tb_kontak_lokasi WHERE id = 1";
$data_kontak = mysqli_fetch_assoc(mysqli_query($koneksi, $query_kontak));

// Mengambil data tradisi dari database
$query_tradisi = "SELECT * FROM tb_tradisi ORDER BY id ASC";
$result_tradisi = mysqli_query($koneksi, $query_tradisi);
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Adat & Tradisi - Budaya Adonara</title>
    ...
    <style>
        /* === PERBAIKAN BUG Z-INDEX MENU MOBILE === */
        .top-nav {
        ...
        .bg-overlay {
            ...
            background-image: url('assets/uploads/foto/bg-tradisi.jpg');
        ...
        .gallery-row {
            ...
            gap: 40px;
            margin-bottom: 80px;
            scroll-margin-top: 120px; /* Jarak scroll dari header */
        }
        ...
        .museum-plaque {
            ...
            border-left: 4px solid #8B2C24; /* Warna aksen tradisi: Merah bata tua */
            padding: 40px;
            border-radius: 4px;
        ...
        .plaque-category {
            ...
            letter-spacing: 3px;
            margin-bottom: 15px;
            border-bottom: 1px solid rgba(139,44,36,0.3);
            padding-bottom: 5px;
        }
...
<body class="dark-editorial-theme">
    ...
    <main class="main-content">
        ...
        <section class="about-section animate-on-scroll" style="border-top: none; padding-top: 60px; padding-bottom: 20px;">
            ...
            <p style="text-align: center; color: var(--text-muted); margin-top: -30px; margin-bottom: 50px; font-size: 14px; letter-spacing: 1px;">Warisan Leluhur yang Terus Dijaga</p>
        </section>

        <div class="container" style="margin-top: 0; padding: 0 15px;">
            <?php while($row = mysqli_fetch_assoc($result_tradisi)): ?>
                <?php 
                // GENERATOR ID ANTI ERROR
                $anchor_id = preg_replace('/[^a-z0-9]+/i', '-', strtolower(trim($row['judul'])));
                $anchor_id = trim($anchor_id, '-'); 
                ?>
                
                <!-- GENERATED ID: <?= $anchor_id; ?> -->
                
                <div class="gallery-row animate-on-scroll" id="<?= $anchor_id; ?>">
                    <div class="artwork-display">
                        <?php $foto = !empty($row['foto_path']) ? $row['foto_path'] : 'assets/uploads/foto/default-tradisi.jpg'; ?>
                        <img src="<?= $foto; ?>" alt="<?= $row['judul']; ?>" class="canvas-frame">
                    </div>
                    <div class="museum-plaque">
                        <span class="plaque-category">Tradisi Leluhur</span>
                        <h3><?= $row['judul']; ?></h3>
                        <p><?= nl2br($row['isi']); ?></p>
                        <?php if(!empty($row['audio_path'])): ?>
                            <div class="audio-guide">
                                <strong>🎧 Narasi Lisan Tradisi</strong>
                        ...
                        <?php else: ?>
                            <div style="margin-top: 25px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.05); text-align: center;">
                                <em style="color: var(--text-muted); font-size: 12px;">*Catatan lisan belum tersedia.</em>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.dropdown-content a').forEach(anchor => {
                anchor.addEventListener('click', function (e) {
                    ...
                    if (href.includes('#') && href.includes('tradisi.php')) {
