...
if (isset($_POST['update'])) {
    ...
    // Simpan path lama sebagai default agar tidak hilang jika tidak diupdate
    $audio_path = $data['audio_path'];
    $foto_path  = $data['foto_path'];

    // 1. CEK & UPLOAD AUDIO BARU
    if (!empty($_FILES['audio_file']['name'])) {
        $nama_audio = $_FILES['audio_file']['name'];
        $tmp_audio = $_FILES['audio_file']['tmp_name'];
        $error_audio = $_FILES['audio_file']['error'];
        ...
        if ($error_audio === 0) {
            if ($ekstensi_audio == 'mp3') {
                ...
                $folder_audio_relatif = "assets/uploads/audio/";
                $folder_audio_fisik = "../" . $folder_audio_relatif;
                
                if (!is_dir($folder_audio_fisik)) {
                    mkdir($folder_audio_fisik, 0777, true);
                }

                $path_audio_tujuan = $folder_audio_fisik . $nama_audio_baru;
                
                if (move_uploaded_file($tmp_audio, $path_audio_tujuan)) {
                    // Hapus file audio lama dari server jika ada
                    if (!empty($data['audio_path']) && file_exists("../" . $data['audio_path'])) {
                        unlink("../" . $data['audio_path']);
                    }
                    $audio_path = $folder_audio_relatif . $nama_audio_baru;
                } else {
                    echo "<script>alert('Gagal memindahkan file audio baru.');</script>";
                }
            } else {
                echo "<script>alert('Format file audio wajib .mp3');</script>";
            }
        }
    }
    ...
    if (!empty($_FILES['foto_file']['name'])) {
        ...
        $error_foto = $_FILES['foto_file']['error'];
        ...
        if ($error_foto === 0) {
            if (in_array($ekstensi_foto, $ekstensi_diperbolehkan)) {
                ...
                $folder_foto_relatif = "assets/uploads/foto/";
                $folder_foto_fisik = "../" . $folder_foto_relatif;
                
                if (!is_dir($folder_foto_fisik)) {
                    mkdir($folder_foto_fisik, 0777, true);
                }

                $path_foto_tujuan = $folder_foto_fisik . $nama_foto_baru;
                
                if (move_uploaded_file($tmp_foto, $path_foto_tujuan)) {
                    // Hapus file foto lama dari server jika ada
                    if (!empty($data['foto_path']) && file_exists("../" . $data['foto_path'])) {
                        unlink("../" . $data['foto_path']);
                    }
                    $foto_path = $folder_foto_relatif . $nama_foto_baru;
                } else {
                    echo "<script>alert('Gagal memindahkan file foto baru.');</script>";
                }
            } else {
                echo "<script>alert('Format file foto wajib JPG, JPEG, PNG, atau WEBP');</script>";
            }
        }
    }

    // UPDATE DATABASE
    $query_update = "UPDATE tb_sejarah SET kategori='$kategori', judul='$judul', isi='$isi', audio_path='$audio_path', foto_path='$foto_path' WHERE id='$id'";
...
<div class="main-content">
    <div class="form-card">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <!-- INPUT GAMBAR BARU -->
            <div class="form-group">
                <label for="foto_file">Update Gambar / Foto (JPG, PNG)</label>
                <input type="file" name="foto_file" id="foto_file" class="form-control" accept="image/jpeg, image/png, image/webp">
                
                <?php if(!empty($data['foto_path'])): ?>
                    <div class="info-box info-box-success">
                        <img src="../<?= $data['foto_path']; ?>" alt="Preview" class="img-preview">
                        <div>
                            <strong>Gambar Saat Ini:</strong> Sudah ada gambar. Biarkan kosong jika tidak ingin diubah.
                        </div>
                    </div>
                <?php else: ?>
                    <div class="info-box">
                        <strong>Gambar Belum Tersedia.</strong> Unggah file gambar untuk melengkapi materi ini.
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="form-group">
                ...
                <?php if(!empty($data['audio_path'])): ?>
                    <div class="info-box info-box-success">
                        <div>
                            <strong>Audio Saat Ini:</strong> Sudah ada file audio. Biarkan kosong jika tidak ingin diubah.
                        </div>
                    </div>
                <?php else: ?>
                    <div class="info-box">
                        <strong>Audio Belum Tersedia.</strong> Unggah file .mp3 jika ingin menambahkan narasi suara.
                    </div>
                <?php endif; ?>
            </div>
